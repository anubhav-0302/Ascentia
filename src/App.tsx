...
function App() {
  ...
  const handlePageNavigation = (page: string) => {
    ...
    setCurrentPage(page.toLowerCase().replace(' & ', '-').replace(' ', '-'));
  };
      ...
      default:
        return (
          <div className="bg-slate-800/40 backdrop-blur-xl rounded-xl p-8 border border-slate-700/50 text-center">
            <i className="fas fa-construction text-4xl text-teal-400 mb-4"></i>
            <h2 className="text-xl font-bold text-white mb-2">{currentPage.charAt(0).toUpperCase() + currentPage.slice(1).replace('-', ' ')}</h2>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 text-white">
      <Sidebar currentPage={currentPage} onNavigate={handlePageNavigation} />
      <Header />
          ...
          <div className="mb-8 flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold text-white mb-2">
                {currentPage === 'dashboard' ? 'Welcome to Ascentia' : currentPage.charAt(0).toUpperCase() + currentPage.slice(1).replace('-', ' ')}
              </h1>
              <p className="text-gray-400">
                {currentPage === 'dashboard' ? 'Your HR Management Platform' : 'Manage your HR operations'}
              </p>
            </div>
            <div className="flex gap-3">
              <button onClick={handleRefreshData} className="px-4 py-2 bg-slate-700/60 text-white rounded-lg hover:bg-slate-700/80 transition-colors duration-200 flex items-center space-x-2">
                <i className="fas fa-sync-alt"></i>
                <span>Refresh</span>
              </button>
              <button onClick={handleExportData} className="px-4 py-2 bg-slate-700/60 text-white rounded-lg hover:bg-slate-700/80 transition-colors duration-200 flex items-center space-x-2">
